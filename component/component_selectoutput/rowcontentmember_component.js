export class RowContentMember {
    ...
    static madenumcontent({ usedata, forcount, datacount, orderlist, targetdom, receiveddom }) {
        ...
        function activeRadioButtonCheck(parentnode) {
            // 모든 radio 버튼 요소를 선택합니다.
            var radioButtons = parentnode.getElementsByTagName('input');

            // 선택된 radio 버튼 요소를 저장할 변수를 선언합니다.
            var selectedOption;

            // 반복문을 사용하여 선택된 radio 버튼 요소를 찾습니다.
        ...
        if (usedata[forcount].childcol != null) {
            ...
            chbox1.onclick = function (jcurrcount) {
                return function () {
                    ...
                    for (var ib = jcurrcount; ib < ifdiv.length; ib++) {
                        //const statusdiv = ifdiv[ib].parentNode.querySelector('.statusdiv').querySelector('.statussubdiv');

                        const selectedOption = activeRadioButtonCheck(ifdiv[ib].parentNode);


                        // 선택된 radio 버튼 요소의 값을 출력합니다.
            ...
            chbox2.onclick = function (jcurrcount) {
                return function () {
                    ...
                    const selectedOption = activeRadioButtonCheck(ifdiv[jcurrcount].parentNode);
                    if (selectedOption == 0) {
